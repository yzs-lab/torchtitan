...
def maybe_build_fp8_linear(
    ...
    try:
        ...
        scaling_type_weight = ScalingType(job_config.training.float8_scaling_type_weight)
        scaling_type_grad_output = ScalingType(job_config.training.float8_scaling_type_grad_output)
...
def maybe_sync_float8_amax_and_scale_history(
    model: nn.Module, job_config: JobConfig
):
    if not (
        job_config.training.enable_float8_linear
